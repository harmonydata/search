...
export default function CompactResultCard({ result: searchresult, isSelected, onClick }: CompactResultCardProps) {
  // If we have ancestors, we're showing the ancestor's card
  const isAncestorCard = Boolean(searchresult.ancestors && searchresult.ancestors.length > 0);
  const isVariableResult = searchresult.extra_data?.resource_type?.includes('variable');
  const isDatasetResult = searchresult.extra_data?.resource_type?.includes('dataset');
  ...
  const resourceType = displayResult?.extra_data?.resource_type || 
                      (displayResult.dataset_schema && displayResult.dataset_schema["@type"]) || 
                      'Unknown Resource Type';
  
  // Extract title, description, and keywords
  const title = displayResult.dataset_schema?.name || displayResult.extra_data?.name || 'Unnamed Resource';
  const description = isAncestorCard && isDatasetResult 
    ? result.dataset_schema?.description || result.extra_data?.description || ''
    : displayResult.dataset_schema?.description || displayResult.extra_data?.description || '';
  
  // Get keywords from either dataset_schema.keywords or result fields
  const unfilteredKeywords: string[] = displayResult.dataset_schema?.keywords || 
            ...
            <>
              {hasVariables && (
                <SquareChip 
                  ...
                  size="small"
                >
                  {hasMatchedVariables ? `${matchedVariablesCount} Matched Variables` : `${variablesCount} Variables`}
                </SquareChip>
              )}
