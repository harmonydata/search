 ...
 */
export function findOrganizationLogo(organizationName?: string, providedLogo?: string): string | undefined {
  // If a logo is directly provided, use that
  if (providedLogo && providedLogo.trim().length > 0) {
    console.log(`Using provided logo for ${organizationName || 'Unknown'}: ${providedLogo}`);
    return providedLogo;
  }
  ...
  if (organizationName.includes(' ') && organizationName.split(' ').every(part => part.length <= 6)) {
    console.log(`Organization name "${organizationName}" appears to be a space-delimited list of abbreviations`);
    // Split the string by spaces and try to find a logo for each part
    const parts = organizationName.split(' ').map(part => part.trim()).filter(part => part.length > 0);
    
    for (const part of parts) {
      // Check if this abbreviation has a direct logo mapping
      if (abbreviationToLogo[part]) {
        console.log(`Found direct logo match for abbreviation "${part}": ${abbreviationToLogo[part]}`);
      ...
      for (const [knownName, abbr] of Object.entries(organizationAbbreviations)) {
        if (knownName.toLowerCase().includes(part.toLowerCase()) || 
            abbr.toLowerCase() === part.toLowerCase()) {
          const logoPath = `/logos/${abbr}.png`;
          console.log(`Found partial match for "${part}" with known organization "${knownName}" (${abbr}): ${logoPath}`);
          return logoPath;
        }
      }
    }
  ...
  if (abbr) {
    const logoPath = `/logos/${abbr}.png`;
    console.log(`Found direct organization match for "${organizationName}": ${logoPath}`);
    return logoPath;
  }
  ...
  if (abbreviationToLogo[organizationName]) {
    console.log(`Found direct logo match for organization abbreviation "${organizationName}": ${abbreviationToLogo[organizationName]}`);
  ...
  for (const [knownName, abbr] of Object.entries(organizationAbbreviations)) {
    if (organizationName.toLowerCase().includes(knownName.toLowerCase()) || 
        organizationName.toLowerCase().includes(abbr.toLowerCase())) {
      const logoPath = `/logos/${abbr}.png`;
      console.log(`Found partial match for "${organizationName}" with known organization "${knownName}": ${logoPath}`);
      return logoPath;
    }
  }
